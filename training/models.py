...
class ModelMaking:
    def make_generator_model_28(self, batch_size, img_size, colors):
    ...
    def make_generator_model_64(self, batch_size, img_size, colors):
        model = tf.keras.Sequential()
        ...
        model.add(tf.keras.layers.Conv2DTranspose(int(batch_size/4), (5, 5), strides=(2, 2), padding='same', use_bias=False))
        assert model.output_shape == (None, int(img_size/4), int(img_size/4), batch_size/4)
        model.add(tf.keras.layers.BatchNormalization())
        model.add(tf.keras.layers.LeakyReLU())

        model.add(tf.keras.layers.Conv2DTranspose(int(batch_size/8), (5, 5), strides=(2, 2), padding='same', use_bias=False))
        assert model.output_shape == (None, int(img_size/2), int(img_size/2), batch_size/8)
        model.add(tf.keras.layers.BatchNormalization())
        model.add(tf.keras.layers.LeakyReLU())
